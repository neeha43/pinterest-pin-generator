...
const Generator: React.FC<GeneratorProps> = ({ onComplete, onBack }) => {
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
          <StepCard title="What do you want this Pinterest pin to achieve?" step={1} total={totalSteps}>
        ...
        return (
          <StepCard title="What are you promoting?" step={2} total={totalSteps}>
            <div className="space-y-4">
              ...
              <div className="pt-2">
                 <label className="block text-sm font-medium text-slate-700 mb-1">Page Title or URL (Optional)</label>
                 <input 
                  type="text" 
                  className="w-full p-3 border border-slate-300 rounded-xl focus:ring-2 focus:ring-red-500 focus:border-red-500 outline-none"
      ...
      case 3:
        return (
          <StepCard title="Which niche best fits your content?" step={3} total={totalSteps}>
            ...
            <div className="mt-4">
               <label className="block text-sm font-medium text-slate-700 mb-1">Or type your niche:</label>
               <input 
                  type="text" 
      ...
      case 4:
         return (
          <StepCard title="Who is this pin for?" step={4} total={totalSteps}>
            <div className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  className="w-full p-3 border border-slate-300 rounded-xl outline-none focus:border-red-500"
                 ...
                 <div className="flex gap-2">
                    {['Any', 'Women', 'Men'].map(g => (
                      <button 
                        ...
                        className={`flex-1 py-2 rounded-lg border text-sm font-medium transition-colors ${answers.audience_profile.gender === g ? 'bg-red-50 border-red-500 text-red-700' : 'border-slate-200 hover:border-slate-300'}`}
                      >
                        {g}
                      </button>
                    ))}
              ...
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Situation or Interest</label>
                <input 
                  type="text" 
      ...
      case 5:
        return (
          <StepCard title="What problem or curiosity should this pin speak to?" step={5} total={totalSteps}>
             <div className="grid gap-3">
              {[
                'I want answers',
            ...
            <div className="mt-4">
               <label className="block text-sm font-medium text-slate-700 mb-1">Or describe in one sentence:</label>
               <input 
                  type="text" 
                  className="w-full p-3 border border-slate-300 rounded-xl outline-none focus:border-red-500"
      ...
      case 6:
        return (
           <StepCard title="Which angle should the pin use?" step={6} total={totalSteps}>
            <div className="grid gap-3">
              {ANGLES.map(angle => (
                <OptionButton 
                  key={angle} 
      ...
      case 7:
        return (
           <StepCard title="Pick a headline style you like:" step={7} total={totalSteps}>
            <div className="grid gap-3">
      ...
      case 8:
        return (
          <StepCard title="What should people do next? (CTA)" step={8} total={totalSteps}>
            <div className="grid gap-3">
              {CTAS.map(cta => (
                <OptionButton 
                  key={cta} 
                   ...
                   <div className="flex gap-2">
                    {['Light', 'Dark', 'Neutral', 'High-contrast'].map(m => (
                      <button 
                        ...
                        className={`flex-1 py-2 rounded-lg border text-xs sm:text-sm font-medium transition-colors ${answers.color_mood === m ? 'bg-slate-800 text-white border-slate-800' : 'border-slate-200 hover:border-slate-300'}`}
                      >
                        {m}
                      </button>
                    ))}
      ...
      case 10:
        return (
           <StepCard title="Enter 1–3 keywords people might search on Pinterest" step={10} total={totalSteps}>
              <div className="space-y-4">
                <input 
                  type="text" 
                  className="w-full p-4 border border-slate-300 rounded-xl outline-none focus:border-red-500 text-lg"
      ...
      case 11:
         return (
           <StepCard title="What do you want me to generate?" step={11} total={totalSteps}>
             <div className="space-y-6">
                <div className="space-y-2">
                   {OUTPUT_OPTIONS.map(opt => (
                     <div key={opt} className="flex items-center gap-3 p-3 border border-slate-200 rounded-xl hover:bg-slate-50 cursor-pointer" onClick={() => {
                   ...
                   <div className="flex gap-4">
                      {[1, 3, 5].map(count => (
                         <button 
                            ...
                            className={`flex-1 py-3 rounded-xl border text-lg font-bold transition-all ${answers.variation_count === count ? 'bg-red-50 border-red-500 text-red-600' : 'border-slate-200 text-slate-500 hover:border-slate-300'}`}
                         >
                            {count}
                         </button>
                      ))}
            ...
            <StepCard title="Ready to generate?" step={12} total={totalSteps}>
               <div className="space-y-6">
                  <div className="bg-slate-50 p-6 rounded-2xl border border-slate-200">
                   ...
                   <button 
                      onClick={handleNext}
                      className="w-full py-4 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold text-lg shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2"
                    >
...
const StepCard: React.FC<{ title: string; children: React.ReactNode; step: number; total: number }> = ({ title, children, step }) => (
  <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-sm border border-slate-100 animate-in fade-in slide-in-from-bottom-4 duration-500">
    <h2 className="text-2xl sm:text-3xl font-bold text-slate-900 mb-8 leading-tight">{title}</h2>
    {children}
  </div>
);
...
const OptionButton: React.FC<{ selected: boolean; onClick: () => void; children: React.ReactNode }> = ({ selected, onClick, children }) => (
  <button
    onClick={onClick}
    className={`w-full p-4 rounded-xl border-2 text-left font-medium transition-all duration-200 flex items-center justify-between group ${
      selected 
