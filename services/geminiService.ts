...
import { WizardAnswers, PinResult, VideoAnalysisResult } from "../types";
...
export const generateImageFromPrompt = async (
  ...
  try {
    if (model.startsWith('imagen')) {
       ...
       const config: any = { imageConfig: { aspectRatio: aspectRatio }, seed: seed };
       if (model === 'gemini-3-pro-image-preview') config.imageConfig.imageSize = '1K';
...
export const editImageWithTextOverlay = async (
  ...
  const prompt = `Edit this image by adding the text: "${text}". Styling: ${stylingInstructions}. Return only the edited image.`;
...
export const generateTextAnimationVideo = async (
  ...
  const ai = getAiClient();
  if (!ai) return MOCK_VIDEO_URL;

  const prompt = `A cinematic video starting from the provided image. The text "${text}" animates into view elegantly. Style/Mood: ${styling}.`;
  ...
  try {
    ...
    console.log("Caught video gen error:", error);
    if (error.message?.includes('403') || error.message?.includes('NOT_FOUND')) return MOCK_VIDEO_URL;
    throw error;
  }
};

// --- NEW: VIDEO ALCHEMIST LOGIC ---

/**
 * Analyzes video frames to determine music mood and generate social content.
 */
export const analyzeVideoFrames = async (frames: string[]): Promise<VideoAnalysisResult> => {
  const ai = getAiClient();
  if (!ai) throw new Error("No API Key");

  // Create parts for the model: prompt + 3 images
  const parts: any[] = [{ text: `
    Analyze these video frames. 
    1. Suggest the perfect background music (Genre, Mood, Tempo, and a specific Search Query).
    2. Write a viral, engaging Voiceover Script (max 30 words) suitable for a Short/Reel.
    3. Generate a catchy Title, Caption, and 5 Hashtags.
    
    Return strict JSON.
  ` }];

  frames.forEach(frame => {
    // Frame is data:image/jpeg;base64,... we need to strip prefix
        ...
        responseSchema: {
          ...
          properties: {
            music: {
               ...
               required: ["mood", "genre", "tempo", "searchQuery", "reasoning"]
            },
            social: {
               type: Type.OBJECT,
               properties: {
